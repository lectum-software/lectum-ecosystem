...
Após a primeira entrega, a operação reforçou que esses gráficos de **Posts** e **Respostas** em comunidades devem representar somente conteúdos publicados por psicólogos, sem misturar posts de pacientes. A cobertura de acolhimento continua sendo o bloco responsável por analisar posts de pacientes e resposta qualificada de psicólogos verificados.
...
- Escopar esses dois gráficos a autores com `user.role="psicologo"` tanto no dashboard geral `/comunidades` quanto no detalhe `/comunidades/[slug]?tab=estatisticas`.
...
- Manter o layout desses gráficos igual ao bloco de publicações da aba **Estatísticas** do psicólogo: título curto, badge de total, pizza à esquerda e legenda vertical textual à direita.

## Consequências

- A operação passa a ver a composição real de posts e respostas de psicólogos no mesmo recorte temporal da cobertura.
- Posts de pacientes deixam de entrar nos gráficos de formato de conteúdo, reduzindo ambiguidade com o bloco **Cobertura de acolhimento**.
...
- Smoke real de service para o dashboard `/comunidades?period=all`: `posts_by_content_format.total=16` e `replies_by_content_format.total=10`, confirmando o recorte somente de psicólogos no banco local.
...
- Browser local/headless com Admin real temporário validou `/comunidades` e `/comunidades/perguntas-da-comunidade-layout?tab=estatisticas` em desktop e mobile ~390px, ambos com **Cobertura de acolhimento**, cards **Posts** e **Respostas**, copy com **psicólogos**, 1 pizza de posts e 1 pizza de respostas; admin temporário removido da base local ao final.
