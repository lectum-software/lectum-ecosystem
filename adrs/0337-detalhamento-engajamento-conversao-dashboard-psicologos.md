...
O bloco **Conversão e engajamento dos psicólogos** agregava o envolvimento comunitário em apenas
...
**Conversão x Engajamento**. Em complemento, a leitura precisou separar psicólogos que nunca engajaram
...
- Estender `profile_conversion_engagement` no dashboard com totais e comparações para
...
- Expandir os quadrantes de **Conversão x Engajamento** de 2 colunas para 4 colunas:
  **Muito engajado**, **Engajado**, **Pouco engajado** e **Sem engajamento**, com o recorte separado
  de **Dados insuficientes**.
- Atualizar o filtro composto `profile_conversion_engagement` da lista Admin para aceitar os novos ids dos
  oito recortes e o recorte de dados insuficientes.
- Renderizar a matriz **Conversão x Engajamento** com o mesmo layout de **Intenção x Engajamento** de
...
- O bloco segue mobile-first: no mobile os cards são agrupados por linha de conversão; no desktop a
...
- Validação estática do build: bundle de `/psicologos` contém **Sem engajamento**, `strong_conversion_no_engagement` e `low_conversion_no_engagement`.
...
- Smoke complementar de layout: `GET http://localhost:3002/psicologos` e `GET http://localhost:3002/psicologos/lista?profile_conversion_engagement=strong_conversion_no_engagement` retornaram 200.
...
A parte desta ADR que mantinha **Dados insuficientes** no donut de **Engajamento** e em **Conversão x Engajamento** foi substituida pela ADR-0338. A partir dela, `profile_conversion_engagement` usa somente os oito cruzamentos reais de conversão x engajamento; psicologos com 0 interacoes reais entram em **Sem engajamento**. **Dados Insuficientes** permanece somente na analise isolada de **Conversão**.
