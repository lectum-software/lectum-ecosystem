# ADR-0329 - Contadores de demanda e engajamento no detalhe Admin do psicologo
...
A aba **Geral** do detalhe administrativo do psicologo exibia cinco contadores independentes: Ranking, Avaliacao, Cliques no WhatsApp, Favoritado e Visualizacoes de perfil. O produto solicitou reduzir a leitura de demanda para um unico contador e adicionar um contador de **Engajamento**.

Ja existia uma fonte real para essa decisao no endpoint `GET /api/admin/private/psychologists/:id/statistics`: `business.demand` traz o resultado individual de demanda e `community.engagement_diagnosis` traz o diagnostico geral de engajamento comunitario.

## Decisao

A aba **Geral** passa a renderizar quatro contadores, nesta ordem: **Ranking**, **Avaliacoes**, **Demanda** e **Engajamento**.
...
- **Demanda** usa `business.demand.label` do endpoint real de estatisticas no periodo `all` como metrica de destaque, sem recalcular a classificacao no frontend.
...
- O resumo fica mais executivo e evita repetir tres cards que pertencem ao mesmo conceito de demanda.
- A UI evita divergencia de regra porque nao reimplementa a classificacao de demanda nem a agregacao de engajamento no cliente.
...
## Atualizacao 2026-07-28 - Demanda sem sinais internos

O produto solicitou remover **WhatsApp**, **Favoritado** e **Visualizacoes de perfil** de dentro do contador **Demanda**. O card passa a exibir somente o resultado real de demanda, mantendo os sinais detalhados fora do resumo da aba **Geral**.
