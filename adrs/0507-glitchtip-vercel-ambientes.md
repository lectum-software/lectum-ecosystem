...
A 0.1.401 demonstrou que os links não eram a causa efetiva (zero links removidos). A inspeção do código oficial `nextjs/adapter-vercel` identificou `filePathMap` e `fileHashes` em `.vc-config.json`, gerados antes do postbuild com caminhos relativos ao `repoRoot` do Next. A limpeza deve podar apenas entradas de mapas removidos da própria `.next`, preservando runtime, referências externas e demais campos. O root vem de `required-server-files.json`, não de suposição sobre monorepo. Para reproduzir, o adapter oficial `@next-community/adapter-vercel@0.0.1-beta.29` e seu utilitário de build `@vercel/build-utils@13.6.2` são usados apenas em diretório temporário de ferramentas; nenhum package/lockfile das aplicações é alterado (política PACKAGES.md).


Resultado: output real do adapter em 0.1.402 validou 318 funções sem referências internas de mapas ou arquivos ausentes. Ambos os deploys Vercel ficaram Ready; versões e CSP publicadas foram conferidas. A remoção de `filePathMap` foi a correção determinante para o empacotamento. Etapas anteriores permanecem registradas como histórico, não como diagnóstico comprovado isoladamente. Production não foi promovida.
