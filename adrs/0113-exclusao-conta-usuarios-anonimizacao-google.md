...
- Exigir confirmação textual `EXCLUIR` e:
  - senha atual para contas não Google com senha local;
  - reautenticação Google recente para contas Google.
- Atualização em 2026-08-15: para exclusão própria, `provider="google"` passa a prevalecer sobre
  senhas locais legadas; contas Google confirmam por reautenticação Google mesmo quando existe hash
  de senha antigo. Ver ADR-0461.
