...
- Complemento operacional em 2026-07-13: a lista Admin passa a expor `user.email` abaixo do nome do psicólogo e remove a coluna **Avaliações** da tabela principal. A decisão substitui a regra anterior de não expor e-mail nesta lista porque o suporte administrativo passou a exigir identificação rápida da conta sem entrar no detalhe; a exposição permanece restrita ao endpoint Admin privado e não inclui senha, telefone, CPF, tokens ou códigos. A tabela também adiciona **Data de cadastro** imediatamente após **Psicólogo**, usando `user.createdAt`, e a busca passa a aceitar e-mail além de nome/CRP.
...
- A operação ganha identificação de conta diretamente na lista, reduzindo cliques para suporte. O custo é maior exposição de e-mail em uma tela administrativa ampla; por isso a decisão fica registrada como exceção operacional ao princípio original de exposição mínima e deve continuar limitada ao app Admin autenticado.
...
- Revalidação operacional em 2026-07-13 para ajuste da lista: `pnpm --dir backend check`, `pnpm --dir backend build`, `pnpm --dir admin check`, `pnpm --dir admin build` e `pnpm check`. Smoke API autenticado com admin temporário real removido ao final em `GET /api/admin/private/psychologists?limit=2&sort=relevance` retornou `200`, `count=7`, `firstHasEmail=true` e `firstHasCreatedAt=true`. Browser local/headless/CDP com admin temporário real removido ao final validou desktop `1440x1000` com cabeçalhos `Rank`, `Psicólogo`, `Data de cadastro`, `Plano`, `Perfil`, `Registro`, `Ações`, sem coluna **Avaliações**, 7 linhas com e-mail e data, sem overflow; mobile `390x844` validou 7 cards com e-mail/data e sem **Avaliações**.
- `pnpm --dir backend check`
