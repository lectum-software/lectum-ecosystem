...
A lista Admin `/psicologos/lista` ja enviava `traction` e `engagement` ao endpoint real quando esses filtros estavam na URL, mas a `queryKey` do TanStack Query nao incluia todos os parametros suportados pela lista. Com isso, mudar somente filtros novos como **Tracao** e **Engajamento** podia reaproveitar o cache da lista sem filtros e deixar linhas incompatíveis visiveis.
...
- `traction`, `engagement` e `traction_engagement`;
