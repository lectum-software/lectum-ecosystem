# ADR-0424: Reset total seguro do ambiente de desenvolvimento

## Status

Accepted para uso local; controles de segurança emendados pelo
...
2. bloqueia execução em qualquer ambiente publicado reconhecido, incluindo homologação, staging e
   produção;
...
6. bloqueia bucket/endpoint R2 com aparência de ambiente publicado, sem variável de contorno; um
   alvo local que não seja inequivocamente descartável exige prefixo também descartável;
...
- As antigas permissões para contornar a proteção de banco ou bucket foram removidas. Este ponto
  substitui qualquer orientação anterior em contrário neste ADR.
