...
O bloco **Demanda e engajamento dos psicólogos** agregava o envolvimento comunitário em apenas
...
**Demanda x Engajamento**. Em complemento, a leitura precisou separar psicólogos que nunca engajaram
...
- Estender `demand_engagement` no dashboard com totais e comparações para
...
- Expandir os quadrantes de **Demanda x Engajamento** de 2 colunas para 4 colunas:
  **Muito engajado**, **Engajado**, **Pouco engajado** e **Sem engajamento**, com o recorte separado
  de **Dados insuficientes**.
- Atualizar o filtro composto `demand_engagement` da lista Admin para aceitar os novos ids dos
  oito recortes e o recorte de dados insuficientes.
- Renderizar a matriz **Demanda x Engajamento** com o mesmo layout de **Intenção x Engajamento** de
...
- Manter **Dados insuficientes** em card compacto separado, pois esse recorte não pertence aos eixos
  de demanda e engajamento.

## Consequências

- O Admin passa a enxergar se a demanda forte aparece entre psicólogos muito engajados, engajados,
...
- O bloco segue mobile-first: no mobile os cards são agrupados por linha de demanda; no desktop a
...
- Validação estática do build: bundle de `/psicologos` contém **Sem engajamento**, `strong_demand_no_engagement` e `low_demand_no_engagement`.
...
- Smoke complementar de layout: `GET http://localhost:3002/psicologos` e `GET http://localhost:3002/psicologos/lista?demand_engagement=strong_demand_no_engagement` retornaram 200.
...
A parte desta ADR que mantinha **Dados insuficientes** no donut de **Engajamento** e em **Demanda x Engajamento** foi substituida pela ADR-0338. A partir dela, `demand_engagement` usa somente os oito cruzamentos reais de demanda x engajamento; psicologos com 0 interacoes reais entram em **Sem engajamento**. **Dados Insuficientes** permanece somente na analise isolada de **Demanda**.
