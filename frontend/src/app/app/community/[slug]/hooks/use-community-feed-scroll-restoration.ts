...
const readCommunityFeedScrollSnapshot = () => {
...
const clearCommunityFeedScrollSnapshot = () => {
...
const resolveRestoreScrollY = (snapshot: CommunityFeedScrollSnapshot) => {
  ...
  if (postElement && snapshot.postViewportTop !== null) {
    const absolutePostTop = postElement.getBoundingClientRect().top + window.scrollY;
...
export const rememberCommunityFeedScrollPosition = (postId: string) => {
  ...
  const sourceHref = currentInternalHref();
...
export const useCommunityFeedScrollRestoration = ({
  ...
  useEffect(() => {
    ...
    const snapshot = readCommunityFeedScrollSnapshot();
    const currentHref = currentInternalHref();
    ...
    const tryRestore = () => {
      ...
      const targetScrollY = resolveRestoreScrollY(snapshot);
