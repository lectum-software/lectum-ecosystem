...
import { z } from "zod";
import type { NotificationPreference, NotificationPrefs } from "@/api/req/notification";
...
export const fromFormValues = (
  ...
  const prefs = SWITCH_CATEGORIES.reduce<NotificationPrefs>(
    (acc, category) => {
      acc[category.key] = {
...
type NotificationPreferencesFormProps = {
  query: QueryObserverResult<NotificationPreference, Error>;
  update: UseMutationResult<NotificationPreference, Error, NotificationPrefs>;
...
export const NotificationPreferencesForm = ({
  ...
  const canEditPreferences = query.isSuccess && Boolean(query.data?.prefs);
  ...
  const handleSubmit = form.hook.handleSubmit((formValues) => {
    if (!canEditPreferences || update.isPending) return;

    update.mutate(fromFormValues(formValues, sessionRole, query.data?.prefs), {
  ...
  return query.isLoading ? (
    ...
    <EmptyState
      description="Não foi possível carregar suas preferências. Tente novamente em instantes."
    ...
    <form className="grid gap-7" onSubmit={handleSubmit}>
      ...
      <div className="sticky bottom-4 z-10 rounded-[1.75rem] bg-background/85 p-2 backdrop-blur md:static md:bg-transparent md:p-0">
        <Button
          ...
          disabled={!canEditPreferences || update.isPending || !form.isDirty || form.isError}
          type="submit"
        >
