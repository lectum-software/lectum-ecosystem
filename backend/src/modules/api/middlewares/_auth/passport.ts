    ...
    new GoogleStrategy(
      ...
      async (req: Request, _accessToken: string, _refreshToken: string, profile: Profile, done) => {
        try {
          ...
          let adultConfirmed: boolean | undefined;
          ...
          if (state) {
            ...
            adultConfirmed =
              state.query.adult_confirmed === undefined
                ? undefined
                : state.query.adult_confirmed === "true";
          ...
          const emailVerified =
            googleEmail?.verified === true || profileJson.email_verified === true;
