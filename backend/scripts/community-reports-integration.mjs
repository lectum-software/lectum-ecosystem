// PostgreSQL/HTTP reais descartáveis; sem provider, mocks, build ou configuração do host.
// node backend/scripts/community-reports-integration.mjs --image=lectum-backend:audit-0.1.327
...
if (args.length !== 1 || !/^--image=lectum-backend:audit-\d+\.\d+\.\d+$/.test(args[0])) {
...
const directory = mkdtempSync(path.join(tmpdir(), "lectum-reports-audit-"));
...
try {
  ...
  console.log("REPORTS_IMAGE", image, imageId);
  ...
  const appConfig = envFile(
    ...
    JSON.stringify({
      ...
      // Constructor-only sentinels required by the global router import; OAuth is never called.
      // This is not the Browser/cookie harness and does not validate any external provider.
  ...
  const probePath = fileURLToPath(new URL("./community-reports-probe.cjs", import.meta.url));
  const result = spawnSync(
    "docker",
    [
      ...base,
  ...
  for (const line of `${result.stdout || ""}\n${result.stderr || ""}`.split("\n")) {
    if (/^(CHECK_OK|INTEGRATION_FAILED|COMMUNITY_REPORTS_POSTGRES_HTTP_OK)( |$)/.test(line))
      console.log(line);
  }
  assert.equal(result.status, 0, "Reports não passou na integração isolada.");
} catch {
  console.error("COMMUNITY_REPORTS_INTEGRATION_FAILED");
  process.exitCode = 1;
} finally {
