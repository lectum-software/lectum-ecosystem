// Execução manual: node backend/scripts/auth-integration.mjs --image=lectum-backend:audit-VERSAO --suite=account-confirmation
// --suite=password-reset é o padrão e preserva o wrapper anterior.
...
const suite = suiteArgument?.slice("--suite=".length) || "password-reset";
const imageArgument = args.filter((arg) => arg !== suiteArgument);
if (
  !Object.hasOwn(suites, suite) ||
  args.length !== (suiteArgument ? 2 : 1) ||
...
try {
  ...
  const probePath = fileURLToPath(new URL(`./${suites[suite]}`, import.meta.url));
  const result = spawnSync(
    "docker",
    [
      ...base,
  ...
  assert.equal(result.status, 0, "Autenticação não passou na integração isolada.");
} catch {
  console.error("AUTH_INTEGRATION_FAILED");
  process.exitCode = 1;
} finally {
