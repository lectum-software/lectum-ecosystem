...
// node backend/scripts/billing-history-integration.mjs --image=lectum-backend:audit-0.1.324
...
const directory = mkdtempSync(path.join(tmpdir(), "lectum-billing-audit-"));
...
try {
  console.log("BILLING_IMAGE", image, docker(["image", "inspect", "--format", "{{.Id}}", image]));
  ...
  const appConfig = envFile(
    ...
    JSON.stringify({
      ...
      GOOGLE_CLIENT_ID_API_USER: "unused-local-billing-audit",
      GOOGLE_CLIENT_SECRET_API_USER: "unused-local-billing-audit",
    }),
  );
  ...
  const probePath = fileURLToPath(new URL("./billing-history-probe.cjs", import.meta.url));
  const result = spawnSync(
    "docker",
    [
      ...base,
  ...
  for (const line of `${result.stdout || ""}\n${result.stderr || ""}`.split("\n")) {
    if (/^(CHECK_OK|INTEGRATION_FAILED|BILLING_HISTORY_POSTGRES_HTTP_OK)( |$)/.test(line))
      console.log(line);
  }
  assert.equal(result.status, 0, "Billing não passou na integração isolada.");
} catch {
  console.error("BILLING_HISTORY_INTEGRATION_FAILED");
  process.exitCode = 1;
} finally {
