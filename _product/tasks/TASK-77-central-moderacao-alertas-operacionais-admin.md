...
- Derivar alertas reais a partir de:
  ...
  - `profile_view_event` e `contact_request.channel=whatsapp` para profissional sem convers?o após adaptação.
...
- Os rótulos de **Pendência** na tabela operacional foram normalizados para **Post sem cobertura**, **Perfis não publicados** e **Sem convers?o**, separados dos rótulos técnicos internos.
...
- O ícone de detalhe abre a rota Admin do conteúdo para **Post sem cobertura** e a rota Admin do psicólogo para **Perfis não publicados**/**Sem convers?o**.
- Não houve alteração de Prisma schema/migrations, package novo, mock, seed ou endpoint simulado; `pnpm --dir backend db:migrate` não se aplica.
...
- [x] **Perfis não publicados** e **Sem convers?o** apontam para o psicólogo real e abrem o detalhe administrativo do psicólogo.
- [x] A alteração é mobile-first, sem `<img>` cru, sem package novo e sem migration.
...
- Pedido do usuário: corrigir o filtro **Tipo** de `/moderacao/operacionais`, que mostrava **Sem convers?o** selecionado mas mantinha linhas de **Post sem cobertura**.
...
- [x] **Sem convers?o** não reutiliza mais o cache de **Todos** ou **Post sem cobertura**.
...
A fila exclusiva `/moderacao/operacionais` deixou de exibir as colunas genericas **Plano** e **Status do perfil**. A tabela permanece compacta, mas agora usa a coluna **Detalhes** para expor o dado operacional acionavel conforme o tipo de pendencia: comunidade e engajamento do paciente em posts sem cobertura; plano e motivo real de inatividade em perfis nao publicados; tempo do profissional na plataforma e criterios de adaptacao ja atendidos em profissionais sem convers?o.
