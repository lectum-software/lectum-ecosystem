...
## Complemento em 2026-06-07: WhatsApp sem autentica��o

- Por decis�o de produto, o n�mero de WhatsApp profissional n�o ser� mais autenticado por SMS nem por WhatsApp no MVP.
- A rota `/app/professional/whatsapp/verify` foi mantida como endere�o existente, mas agora � apenas a etapa de informar e salvar o WhatsApp profissional.
- O backend salva o n�mero normalizado em `psychologist_profile.whatsapp` e mant�m `whatsapp_verified_at=null`.
- O contato do paciente continua gerando o link `wa.me` internamente apenas depois do consentimento e da persist�ncia de `contact_request`.
- O campo `whatsapp_verified_at` permanece no banco por compatibilidade hist�rica, mas n�o bloqueia mais `whatsapp_available` nem a cria��o do link de contato.
