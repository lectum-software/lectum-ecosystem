...
Complemento 2026-06-26: a listagem `GET /api/private/notification/index` pode retornar o campo derivado `actor` apenas para `novo_post` e `nova_resposta`, hidratado a partir de `message_props.post_id` ou `message_props.reply_id`. `actor` contém `{ id, name, avatar, role, professional_label, verified, anonymous, deleted }` para apresentação in-app; não altera o schema Prisma. Autores anônimos de posts devem usar o mesmo alias `Membro Anônimo #1234` derivado de `author_id`, com `id=null` e `avatar=null`. Psicólogos podem receber `verified=true` quando o perfil profissional tiver verificação atual equivalente à comunidade, mas a central não exibe sufixos como `· Psicólogo`; o selo verificado é suficiente para diferenciar profissionais. Interações passivas (`upvote`, `salvamento`, `compartilhamento` etc.) permanecem sem identificação de autor.
