...
Complemento 2026-06-26: a listagem `GET /api/private/notification/index` pode retornar o campo derivado `actor` apenas para `novo_post` e `nova_resposta`, hidratado a partir de `message_props.post_id` ou `message_props.reply_id`. `actor` contém `{ id, name, avatar, role, professional_label, anonymous, deleted }` para apresentação in-app; não altera o schema Prisma. Autores anônimos de posts devem usar o mesmo alias `Membro Anônimo #1234` derivado de `author_id`, com `id=null` e `avatar=null`. Psicólogos podem receber `professional_label` (`Psicóloga`, `Psicólogo` ou `Psicólogo(a)`); membros comuns não recebem sufixo visual. Interações passivas (`upvote`, `salvamento`, `compartilhamento` etc.) permanecem sem identificação de autor.
