...
| Fase | Admin |
| Status | Completed |
| DependÃªncias | TASK-45, TASK-46, TASK-60 |
| ADR alvo | ADR sobre exposiÃ§Ã£o administrativa de dados pessoais de pacientes |

## Contexto

A tela de detalhe do paciente usa como referÃªncia `_product/proto/admin/Pacientes/Pacientes - Detalhes.png`.

Esta tela Ã© operacional e deve ser **somente leitura** na V1. O Admin visualiza perfil resumido, engajamento, atividade recente, comunidades e horÃ¡rios de maior atividade quando houver fonte real.

DecisÃµes de produto:

- NÃ£o implementar bloqueio ou silenciamento de paciente.
- NÃ£o implementar taxa de retenÃ§Ã£o.
- NÃ£o criar aÃ§Ãµes administrativas destrutivas.
- NÃ£o expor dados sensÃ­veis alÃ©m do necessÃ¡rio para operaÃ§Ã£o.

## Objetivo

Criar a tela de detalhe administrativo do paciente com dados reais e uma leitura simples de engajamento, sem moderaÃ§Ã£o, sem retenÃ§Ã£o e sem aÃ§Ãµes de bloqueio/silenciamento.

## PrÃ©-requisitos e bloqueios

- TASK-60 concluÃ­da com navegaÃ§Ã£o da lista para o detalhe.
- Ler `ARCHITECTURE.md`, `DATA-MODEL.md`, `PACKAGES.md` e `PROTO-INVENTORY.md`.
- Usar `_product/proto/admin/Pacientes/Pacientes - Detalhes.png` como referÃªncia visual local.
...
- Renderizar:
  - botÃ£o "Voltar para pacientes";
  - cabeÃ§alho com avatar, nome, ID do paciente, status bÃ¡sico, e-mail, gÃªnero, localizaÃ§Ã£o agregada quando disponÃ­vel, data de cadastro e origem de cadastro;
  - cards de engajamento;
  - grÃ¡fico de engajamento por perÃ­odo;
  - lista de atividade recente;
  - comunidades mais ativas;
  - heatmap de horÃ¡rios de maior atividade, quando houver dados reais.
- Status:
  - usar apenas `Ativo`/`Inativo` baseado em `user.active`;
  - nÃ£o mostrar "Bloqueado" ou "Silenciado".
- Menu de trÃªs pontos:
  - omitir se nÃ£o houver aÃ§Ãµes reais e seguras;
  - nÃ£o incluir bloquear, silenciar, deletar ou moderar.
...
- Retornar dados reais de:
  - `user`;
  - `patient_profile`;
  - `visitor_location` somente para localizaÃ§Ã£o agregada/coarse quando houver;
  - `community_member`;
  - `community_post`;
  - `post_reply`;
  - `post_vote`;
  - `post_save`;
  - `post_reply_save`;
  - `professional_review` quando o paciente for autor de avaliaÃ§Ã£o, somente se necessÃ¡rio para atividade recente e respeitando regras de privacidade.
...
- ModeraÃ§Ã£o de publicaÃ§Ãµes, comentÃ¡rios, votos ou avaliaÃ§Ãµes.
- Taxa de retenÃ§Ã£o.
- Exibir localizaÃ§Ã£o precisa.
- Exibir telefone, nascimento, bio ou dados sensÃ­veis sem decisÃ£o explÃ­cita.
- Criar novo modelo de auditoria.
- Criar tracking novo apenas para preencher a tela.

## Contrato tÃ©cnico detalhado

MÃ©tricas de engajamento V1:

- **Posts criados**: contagem de `community_post.author_id = patient.id`.
- **ComentÃ¡rios**: contagem de `post_reply.author_id = patient.id`.
- **Upvotes recebidos**:
  - votos positivos em posts do paciente;
  - votos positivos em respostas do paciente;
  - se nÃ£o for possÃ­vel cobrir ambos com seguranÃ§a, retornar nota de cobertura.
- **Downvotes recebidos**:
...
- **Respostas recebidas**:
  ...
  - replies encadeadas para respostas do paciente quando houver relaÃ§Ã£o confiÃ¡vel.

Atividade recente:

- Derivar apenas de eventos reais existentes:
  - post criado;
  - comentÃ¡rio/resposta criada;
  - voto realizado;
  - post salvo;
  - resposta salva;
  - entrada em comunidade;
  - avaliaÃ§Ã£o criada;
  - login apenas se houver fonte real de sessÃ£o/login.
- NÃ£o exibir "fez login" se nÃ£o existir evento confiÃ¡vel de login/sessÃ£o.

Comunidades mais ativas:

- Combinar participaÃ§Ã£o (`community_member`) com interaÃ§Ãµes do paciente em cada comunidade.
- Mostrar contagem de interaÃ§Ãµes no perÃ­odo.
- Exibir "seguindo"/"membro" somente se vier de `community_member` real.

Heatmap:

- Calcular por dia da semana e hora a partir de `createdAt` de posts, comentÃ¡rios, votos e salvamentos.
- Usar fuso `America/Sao_Paulo`/BrasÃ­lia na agregaÃ§Ã£o e informar a referÃªncia na UI.
- Se nÃ£o houver eventos suficientes, exibir estado vazio honesto.

Privacidade/LGPD:

- E-mail pode ser exibido para admin autenticado se aprovado no ADR da task.
- LocalizaÃ§Ã£o deve ser coarse e derivada de `visitor_location` ou omitida.
- NÃ£o expor IP, coordenadas, endereÃ§o completo, telefone, data de nascimento ou bio nesta V1.

Frontend esperado:

- Reutilizar shell Admin e componentes existentes.
- Mobile-first:
  - cabeÃ§alho empilhado em mobile;
  - cards em grid responsivo;
  - listas legÃ­veis sem tabela horizontal obrigatÃ³ria.
- `Image` de `next/image` para avatar.
- Filtro de perÃ­odo com RHF/Zod/controllers se implementado como form.

## CritÃ©rios de aceite

- [x] Rota de detalhe sÃ³ abre para admin autenticado.
...
- [x] NÃ£o hÃ¡ mÃ©trica de retenÃ§Ã£o.
- [x] Status usa apenas `Ativo`/`Inativo` baseado em fonte real.
- [x] MÃ©tricas de engajamento usam dados reais.
- [x] Atividade recente lista apenas eventos com fonte confiÃ¡vel.
- [x] "Fez login" sÃ³ aparece se houver evento real de login/sessÃ£o.
- [x] Comunidades mais ativas sÃ£o calculadas por interaÃ§Ãµes reais.
- [x] Heatmap usa eventos reais e informa fuso horÃ¡rio.
- [x] Dados sensÃ­veis sÃ£o omitidos ou tratados conforme ADR.
- [x] UI mobile-first validada.
- [x] Nenhum `<img>` cru foi usado.
- [x] `_product/proto/admin/Pacientes/Pacientes - Detalhes.png` foi citada como referÃªncia visual.
- [x] Nenhum mock, dado fake permanente ou endpoint simulado foi usado.
- [x] Checks/builds relevantes executados sem erros.
- [x] ADR criado/atualizado sobre exposiÃ§Ã£o de dados pessoais do paciente.
...
- Implementado endpoint admin privado `GET /api/admin/private/patients/:id?from=YYYY-MM-DD&to=YYYY-MM-DD` com autenticaÃ§Ã£o admin real.
- Implementada tela protegida em `admin/src/app/(admin)/pacientes/[id]` usando o shell Admin existente.
- A tela Ã© somente leitura, sem menu de aÃ§Ãµes destrutivas, sem bloqueio/silenciamento/banimento/exclusÃ£o/moderaÃ§Ã£o e sem mÃ©trica de retenÃ§Ã£o.
- Status do paciente deriva exclusivamente de `user.active` e exibe apenas `Ativo`/`Inativo`.
- MÃ©tricas de engajamento usam dados reais de `community_post`, `post_reply`, `post_vote`, `post_save`, `post_reply_save`, `community_member` e `professional_review`.
- Atividades recentes sÃ£o derivadas somente de eventos reais existentes; login nÃ£o Ã© exibido porque nÃ£o hÃ¡ evento confiÃ¡vel de login/sessÃ£o para esta V1.
- Comunidades mais ativas combinam vÃ­nculo real de `community_member` com interaÃ§Ãµes reais do paciente.
- Heatmap usa `createdAt` de eventos reais e informa o fuso `America/Sao_Paulo`/BrasÃ­lia.
- Dados sensÃ­veis foram omitidos conforme ADR-0241: sem telefone, nascimento, bio, endereÃ§o completo, IP ou coordenadas.
- UI implementada mobile-first com grids responsivos, cards empilhados em mobile e `Image` de `next/image` para avatar.
- ReferÃªncia visual usada: `_product/proto/admin/Pacientes/Pacientes - Detalhes.png`.
- Builder/Quick Copy nÃ£o esteve disponÃ­vel como ferramenta neste ambiente; a limitaÃ§Ã£o foi registrada no ADR-0241 e na prÃ³pria UI.
- NÃ£o houve alteraÃ§Ã£o em `backend/prisma/schema.prisma` nem em migrations; portanto `db:migrate` nÃ£o se aplicou.

## ValidaÃ§Ãµes executadas

- `pnpm --dir backend check` â€” OK.
- `pnpm --dir backend build` â€” OK.
- `pnpm --dir admin check` â€” OK.
- `pnpm --dir admin build` â€” OK.
- `pnpm check` â€” OK.
- Service `showAdminPatient` validado contra paciente real existente no banco local (`demo-patient-reviewer-01`) â€” OK.
- Range invÃ¡lido maior que 90 dias validado retornando `400 invalid_analytics_date_range` â€” OK.
- Endpoint HTTP sem token admin validado retornando `401` â€” OK.
- Rota local Admin `/pacientes/demo-patient-reviewer-01` validada com HTTP `200` no dev server â€” OK.
...
## Ajuste pos-feedback 2026-07-20 - CorreÃ§Ã£o de encoding dos textos

- Pedido do usuÃ¡rio: ajustar textos quebrados exibidos no detalhe administrativo de paciente.
- Corrigidos literais UTF-8 corrompidos no Admin em `/pacientes/[id]`, incluindo abas, mÃ©tricas, cards de situaÃ§Ã£o, tabelas, estados vazios e mensagens de formulÃ¡rio.
- Corrigidos labels/descriptions retornados pelo backend em `GET /api/admin/private/patients/:id`, para que o contrato nÃ£o entregue copies com mojibake.
...
- NÃ£o houve alteraÃ§Ã£o de contrato HTTP, schema Prisma, migration, package, seed, mock, endpoint simulado ou regra de domÃ­nio.
- Builder/Quick Copy nÃ£o estÃ¡ exposto como ferramenta callable no ambiente; a referÃªncia auditÃ¡vel foi a captura enviada pelo usuÃ¡rio e o inventÃ¡rio local `_product/tasks/PROTO-INVENTORY.md`.
- ADR criado: `adrs/0292-correcao-encoding-copy-ui-admin.md`.

### ValidaÃ§Ã£o complementar executada
...
- O seletor de **Periodo** da aba Estatisticas usa os mesmos presets operacionais do psicologo (**Hoje**, **Esta semana**, **Este mes**, **Este ano** e **Todo o periodo**) e mantÃ©m **Personalizado** apenas como estado tecnico quando o admin edita datas manuais.
